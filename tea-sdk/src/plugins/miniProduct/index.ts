/**
 * Created by lingxufeng@bytedance.com from PGC-FE on 2018/5/17.
 * 给微信开发的 tea-web-sdk
 * 一次成型之后，后续修改的概率非常小，研发思路：
 * - 简单: 尽量少写代码
 * - 解耦： copy 主程序中的代码，而不是引用进来复用
 */
declare const wx;
...
import * as predefineEvents from './predefineEvent'
...
class MiniProduct {
    ...
    constructor() {
        ...
        this.addPredefinedEvent();
    }

    // 暴露预设事件方法
    addPredefinedEvent = () => {
        for(let methed in predefineEvents){
            this[methed]=predefineEvents[methed].bind(null, this.event)
        }
    }
...
export default new MiniProduct();
